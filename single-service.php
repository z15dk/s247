		...
		<div class="service-body__grid">
			<div class="service-body__content page-content">
				...
				if ( ! empty( $inline_examples ) ) {
					...
					// Indsæt midt i artiklen (efter ca. halvdelen af afsnittene).
					$parts = explode( '</p>', $_content );
					$total = count( $parts );
					if ( $total >= 4 ) {
						$insert = max( 1, (int) floor( $total / 2 ) );
						$parts[ $insert ] .= '</p>' . $teaser_html;
						$_content = implode( '</p>', $parts );
						// Fjern den ekstra '</p>' vi tilføjede til sidste tomme part.
						$_content = preg_replace( '#</p>$#', '', $_content );
					} else {
						// Kort artikel — læg teaseren efter indholdet.
